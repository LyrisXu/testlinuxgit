...
void saveAvg(int generation){
  ...
  ss<<generation;
  ss<<".txt";
  std::string fileName = "/home/zqshi/BoxWalkExperimentData/Statistics";
  fileName+=ss.str();
...
void saveFile(int generation){
  ...
  ss<<generation;
  ss<<".txt";
  std::string fileName = "/home/zqshi/BoxWalkExperimentData/Generation";
  fileName+=ss.str();
  out.open (fileName.c_str());
...
void readFile(){
  ...
  std::stringstream ss;
  std::ifstream fin2("/home/zqshi/BoxWalkExperimentData/config.txt");
  ...
  std::ifstream fin3("/home/zqshi/BoxWalkExperimentData/config2.txt");
  index=0;
  ...
  ss<<"/home/zqshi/BoxWalkExperimentData/Generation";
  ss<<START_GENERATION;
  ss<<".txt";
  ...
  if(generation_index==0){
    while (true){
      ...
      if(n<ONE_CHUNK_SIZE-3){
        ...
        //population[i][m][n] = static_cast <float> (rand()%(randomRange*2)-randomRange);
        // joint_limitions[n][0] + 
        // static_cast <float> (rand()) /( 
        //   static_cast <float> (RAND_MAX/(joint_limitions[n][1]-joint_limitions[n][0]))
        //   );
...
void mutation(){
    ...
    for(int m=0;m<EXPTIME;m++){
      for(int n=0;n<ONE_CHUNK_SIZE-3;n++){
          if(rand()%1024/1024<mutation_rate){
           ...
           //population[i][m][n] = joint_limitions[n][0] + static_cast <float> (rand()) /( static_cast <float> (RAND_MAX/(joint_limitions[n][1]-joint_limitions[n][0])));
           population[i][m][n] = (joint_limitions[n][0]+joint_limitions[n][1])/2 -(joint_limitions[n][1]-joint_limitions[n][0])*randomRange/2 + static_cast <float> (rand()) /( static_cast <float> (RAND_MAX/(joint_limitions[n][1]*randomRange-randomRange*joint_limitions[n][0])));
            // population[i][m][n] = static_cast <float> (rand()%(randomRange*2)-randomRange);
          }
  ...
  mutation_rate=0.05f*TARGET_EXP_TIME/temp;
    ...
    for(int n=0;n<EXPTIME;n++){
      for(int m=0;m<ONE_CHUNK_SIZE-3;m++){
          if(rand()%8>=5){
            ...
            // population[i][n][m]=population[parent_index1][n][m]*0.7+0.3*population[parent_index2][n][m];
            // population[i+1][n][m]=population[parent_index1][n][m]*0.5+0.5*population[parent_index2][n][m];
          }
          else{
...
void selection(){
  ...
  //     for(int j=population[i][0][ONE_CHUNK_SIZE-3];j<population[i][0][ONE_CHUNK_SIZE-3]*2;j++){
  //       //Exchange left arm and right arm info
  //       if(j>=EXPTIME)
  //         break;
  //       for(int n=0;n<ONE_CHUNK_SIZE-3;n++)
  //         population[i][j][n]=population[i][j-population[i][0][ONE_CHUNK_SIZE-3]][n];
...
void fitCal(){
    ...
    for(int n=0;n<POPULATION_SIZE;n++){
      ...
      population[n][0][ONE_CHUNK_SIZE-1]=population[n][0][ONE_CHUNK_SIZE-3]+BONUS_FOR_DISTANCE*population[n][0][ONE_CHUNK_SIZE-2];
...
void linkstatesCallback(const gazebo_msgs::LinkStates::ConstPtr& msg){
  ...
  if(msg->pose[1].position.z>0.5){ //1.1){
    double x = 0;
  ...
  if(msg->pose[1].position.z<0.4){//1.0){
    ...
    if(distance_best<population[pop_index][0][ONE_CHUNK_SIZE-2])
      distance_best=population[pop_index][0][ONE_CHUNK_SIZE-2];
    resetExp();
  }
}
